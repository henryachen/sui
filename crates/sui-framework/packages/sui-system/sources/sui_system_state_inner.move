...
module sui_system::sui_system_state_inner {
    ...
    public(package) fun store_execution_time_estimates(self: &mut SuiSystemStateInnerV2, estimates: Vec<u8>) {
        if (bag::contains(&mut self.extra_fields, EXTRA_FIELD_EXECUTION_TIME_ESTIMATES_KEY)) {
            bag::remove(&mut self.extra_fields, EXTRA_FIELD_EXECUTION_TIME_ESTIMATES_KEY);
        };
